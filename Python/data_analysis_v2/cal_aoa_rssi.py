...
while(True):
    ...
    if(p2p(point_p, objection_point) <= 3 * diverge_range):
...
for index, point in enumerate(diverge_points):
    ...
    temp_error = 0
    # a 37
    R = np.power(10, ((a_37_p0 - a_37_rssi) / a_37_gamma))
    temp_error += abs(R - p2p(point, a_pos)) ** 2
    #print(R,  p2p(point, a_pos), R - p2p(point, a_pos))
    # a 38
    R = np.power(10, ((a_38_p0 - a_38_rssi) / a_38_gamma))
    temp_error += abs(R - p2p(point, a_pos)) ** 2
    # a 39
    R = np.power(10, ((a_39_p0 - a_39_rssi) / a_39_gamma))
    temp_error += abs(R - p2p(point, a_pos)) ** 2
    # b 37
    R = np.power(10, ((b_37_p0 - b_37_rssi) / b_37_gamma))
    temp_error += abs(R - p2p(point, b_pos)) ** 2
    # b 38
    R = np.power(10, ((b_38_p0 - b_38_rssi) / b_38_gamma))
    temp_error += abs(R - p2p(point, b_pos)) ** 2
    # b 39
    R = np.power(10, ((b_39_p0 - b_39_rssi) / b_39_gamma))
    temp_error += abs(R - p2p(point, b_pos)) ** 2
    # c
    R = np.power(10, ((c_p0 - c_rssi) / c_gamma))
    temp_error += abs(R - p2p(point, c_pos)) ** 2 *3
    # d
    R = np.power(10, ((d_p0 - d_rssi) / d_gamma))
    temp_error += abs(R - p2p(point, d_pos)) ** 2 *3
